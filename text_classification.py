...
def vectorize_text(corpus: List[str]) -> Tuple[TfidfVectorizer, List[List[float]]]:
    """
...
def train_text_classifier(X: List[List[float]], y: List[str]) -> MultinomialNB:
    """
...
def main(corpus: List[str], labels: List[str]) -> None:
    ...
    classifier = train_text_classifier(X_train_vec, y_train)
